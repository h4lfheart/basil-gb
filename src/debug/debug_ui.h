...
struct DebugUI {
    ...
    static constexpr int VRAM_BANK_SIZE = 0x2000;
...
private:
    ...
    uint32_t rgb555_to_abgr(uint16_t rgb555) const;
    void decode_tile(const uint8_t* vram, int tile_id, const uint32_t* palette, uint32_t* out, int atlas_w) const;
    void build_tile_atlas(const uint8_t* vram, const uint32_t* palette, uint32_t* pixels) const;
    void build_tilemap(const uint8_t* bank0, const uint8_t* bank1, bool use_9c00, bool signed_addr,
                       bool cgb, const uint32_t bg_palette[8][4], uint32_t* pixels) const;
    void build_lcd(const uint16_t* framebuffer, uint32_t* pixels) const;
};